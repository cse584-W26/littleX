#!/usr/bin/env bash
#
# Deploy littleX_postgres_handtuned (Flask + raw psycopg + tuned Postgres)
# into the same Kubernetes namespace where jac-scale runs, then start a
# port-forward on localhost:8080 so the benchmark scripts can hit it via
# --url.
#
# This is the "Option 1" fairness setup: every backend (Jac, Neo4j,
# fair-ORM SQLAlchemy, hand-tuned Postgres) takes the same path
#   bench host -> kubectl port-forward -> ClusterIP svc -> pod
# so latency differences reflect the backend, not the deployment topology.
#
# No Docker build. The Flask source is shipped as a ConfigMap and an
# initContainer pip-installs psycopg+flask at startup using the stock
# python:3.12-slim image.
#
# Style mirrors deploy_sqlalchemy_k8s.sh and deploy_neo4j_k8s.sh.
#
# Usage:
#   ./deploy_handtuned_k8s.sh
...
if [ "$EXISTING" -gt 0 ]; then
    echo "littleX_handtuned deployment already running:"
    kubectl get pods -n "$NAMESPACE" -l "$APP_LABEL"
    echo "Run ./teardown_handtuned_k8s.sh first if you want a fresh deployment."
    exit 1
fi

# ---------------------------------------------------------------------------
# Create the source ConfigMap. Flat keys map to nested paths via the
# manifest's `items: ... path:` mappings.
# ---------------------------------------------------------------------------
...
if lsof -ti:${LOCAL_HTTP_PORT} &>/dev/null; then
    ...
    echo "  bench host -> kubectl port-forward -> svc/littlex-handtuned -> flask pod"
    echo "                                                                  |"
    echo "                                                                  v"
    echo "                                                          svc/littlex-handtuned-postgres -> postgres pod"
    echo ""
